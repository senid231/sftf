...
from sftf.SCException import HFHException
from sftf.TestCase import TestResult
from sftf import Log
            ...
            while (vlen > 0):
                ...
                if (name.lower() == "username"):
                    if not (name == "username"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'username' doesnt conform to BNF in RFC3261"))
                    if not (nameval.startswith('"') and nameval.endswith('"')):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'username' value is not quoted. This is not BNF conform"))
                elif (name.lower() == "realm"):
                    if not (name == "realm"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'realm' doesnt conform to BNF in RFC3261"))
                    if not (nameval.startswith('"') and nameval.endswith('"')):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'realm' value is not quoted. This is not BNF conform"))
                elif (name.lower() == "nonce"):
                    if not (name == "nonce"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'nonce' doesnt conform to BNF in RFC3261"))
                    if not (nameval.startswith('"') and nameval.endswith('"')):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'nonce' value is not quoted. This is not BNF conform"))
                elif (name.lower() == "uri"):
                    if not (name == "uri"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'uri' doesnt conform to BNF in RFC3261"))
                    if not (nameval.startswith('"') and nameval.endswith('"')):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'uri' value is not quoted. This is not BNF conform"))
                elif (name.lower() == "response"):
                    if not (name == "response"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'response' doesnt conform to BNF in RFC3261"))
                    if not (nameval.startswith('"') and nameval.endswith('"')):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'response' value is not quoted. This is not BNF conform"))
                elif (name.lower() == "algorithm"):
                    if not (name == "algorithm"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'algorithm' doesnt conform to BNF in RFC3261"))
                    if nameval.startswith('"') and nameval.endswith('"'):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'algorithm' value is qutoed. This is not BNF conform"))
                elif (name.lower() == "cnonce"):
                    if not (name == "cnonce"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'cnonce' doesnt conform to BNF in RFC3261"))
                    if not (nameval.startswith('"') and nameval.endswith('"')):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'cnonce' value is not quoted. This is not BNF conform"))
                elif (name.lower() == "opaque"):
                    if not (name == "opaque"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'opaque' doesnt conform to BNF in RFC3261"))
                    if not (nameval.startswith('"') and nameval.endswith('"')):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'opaque' value is not quoted. This is not BNF conform"))
                elif (name.lower() == "qop"):
                    if not (name == "qop"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'qop' doesnt conform to BNF in RFC3261"))
                    if nameval.startswith('"') and nameval.endswith('"'):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'qop' value is quoted. This is not BNF conform"))
                elif (name.lower() == "nc"):
                    if not (name == "nc"):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'nc' doesnt conform to BNF in RFC3261"))
                    if nameval.startswith('"') and nameval.endwith('"'):
                        res.append(TestResult(TestResult.TR_CAUT,
                                              "Authorization parameter 'nc' value is quoted. This is not BNF conform"))
                else:
                    rem.append(oldv)
                    Log.logDebug("Authorization.parse(): unsupported param", 4)
                vlen = len(v)
