...
from sftf import Config, XMLEventHandler, Log, Helper
import Helper
    ...
    def run(self):
        ...
        for i in self.test * self.numIterations:
            ...
            if (i.name is None) or (i.description is None) or (i.transport is None) or (i.isClient is None):
                Log.logDebug(
                    "SFTF.run(): mandatory configuration (name, description, transport and isClient) is incomplete, ignoring " + str(
                        i), 1)
                Log.logTest("ignoring " + str(i) + " because of incomplete configuration")
            else:
                if (not i.isClient) and (not i.interactRequired):
                    Log.logDebug("SFTF.run(): WARNING: test " + str(
                        i.name) + " is server and interaction is not required: fixing", 1)
                ...
                if (i.minAPIVersion is not None) and (TestCase.TC_API_VERSION < i.minAPIVersion):
                    Log.logTest("'" + i.name + "' required minimum API version (" + str(
                        i.minAPIVersion) + ") is above your API version (" + str(TestCase.TC_API_VERSION) + ")")
                ...
                if (i.maxAPIVersion is not None) and (TestCase.TC_API_VERSION > i.maxAPIVersion):
                    Log.logTest(
                        "'" + i.name + "' required maximum API version is below your API version (" + TestCase.TC_API_VERSION + ")")
        ...
        for j in self.test:
            ...
            Log.logTest("===================")
            if 'MediaSockets' in Config.resources:
                for sp in Config.resources['MediaSockets']:
                    sp[0].close()
                    sp[1].close()
                Config.resources['MediaSockets'] = []
            if 'XMLEH' in Config.resources and j.name in Config.resources['XMLEH']:
...
def main():
    ...
    sc = SFTF()
    try:
        opts, args = getopt.getopt(sys.argv[1:], "acCdD:hiIn:rRsSt:u:V",
                                   ["all", "client", "debug", "directory", "help", "interactive", "no-color",
                                    "non-interactive", "no-register", "no-summary", "register", "server", "testcase=",
                                    "test-suite=", "version"])
    ...
    if (Config.LOCAL_IP == "") and (Config.LOCAL_HOSTNAME == ""):
        ...
        try:
            ...
            adrlist = socket.getaddrinfo(Config.LOCAL_HOSTNAME, Config.LOCAL_PORT, socket.AF_UNSPEC, socket.SOCK_DGRAM,
                                         0)
            Config.LOCAL_IP = adrlist[0][4][0]
        except socket.gaierror:
            print("Failed to resolve LOCAL_HOSTNAME ('" + str(
                Config.LOCAL_HOSTNAME) + "') from Config.py\r\nPlease fix the hostname or specify the IP as LOCAL_IP in Config.py")
            sys.exit(1)
    checkIPv6Support()
    sc.run()
    return sc
