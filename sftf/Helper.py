...
def createDummyBody():
    ...
    """
    return ['v=0\r\n', \
            'o=' + str(Config.SC_USER_NAME) + ' ' + str(int(time.time())) + ' ' + str(
                int(time.time())) + ' IN IP4 ' + str(Config.LOCAL_IP) + '\r\n', \
            's=Dummy on hold SDP\r\n', \
            'c=IN IP4 0.0.0.0\r\n', \
...
def get_req_hfh_dict(method):
    ...
    req_hfh_dict = {'invite': ['Call-ID', 'CSeq', 'Contact', 'Content-Length', \
                    ...
                    'refer': ['Allow', 'Call-ID', 'Contact', 'CSeq', 'From', \
                              'Max-Forwards', 'To', 'Via']}
    if method.lower() in req_hfh_dict:
...
def createClassInstance(name):
    ...
    if mod:
        try:
            cs = getattr(mod, name)
        except AttributeError as param:
...
def usrlocGetContacts(aor):
    if aor.lower().startswith("sip:"):
        ...
        aor = aor[5:]
    if hasattr(Config, "usrloc") and aor in Config.usrloc:
