...
class Transaction:
            ...
            if self.isClient:
                if message.code < 200:
                    ...
                    if self.dialog.remoteUri.tag is None:
                        if message.hasParsedHeaderField("To"):
                            ...
                            Log.logDebug(
                                "Transaction.appendMessage(): missing parsed To header, using raw (which can be empty)",
                                2)
                    ...
                    if message.hasHeaderField("Contact") and message.code < 300:
                        if message.hasParsedHeaderField("Contact"):
                            ...
                            Log.logDebug(
                                "Transaction.appendMessage(): missing parsed Contact header, using raw (which can be empty)",
                                2)
                ...
                if message.code < 200:
                    ...
                    if message.hasParsedHeaderField("To"):
                        ...
                        Log.logDebug(
                            "Transaction.appendMessage(): missing parsed To header, using raw (which can be empty)", 2)
    ...
    def createReply(self, code, reason, req=None, createEvent=True):
        ...
        """
        Log.logDebug("Transaction.createReply(): entered with code=\'" + str(code) + "\', reason=\'" + str(
            reason) + "\', createEvent=\'" + str(createEvent) + "\'", 5)
        if req is not None:
