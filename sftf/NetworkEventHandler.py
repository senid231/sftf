...
from sftf import SipEvent as SE, Config, Log
...
class NetworkEventHandler(EventHandler):
    ...
    def __init__(self, transp=None, host=None, port=None, addResource=True, createSock=True, eventType=SE.SipEvent):
        ...
        if addResource and 'NEH' in Config.resources and key in Config.resources['NEH']:
            Log.logDebug(
                "NetworkEventHandler.init(): closing NEH which uses the same address (key '" + key + "') to prevent bind error",
                3)
    ...
    def createSock(self, _transp, _ip, _port):
        ...
        try:
            ...
            Log.logDebug("NetworkEventHandler.createSock(): failed to bind " + str(_transp) + " socket to " + str(
                _ip) + ":" + str(_port), 1)
            raise inst
        self.addSock(sock)
        ...
        if (self.transp == socket.SOCK_DGRAM):
            ...
            if len(s_in) > 0:
                try:
                    ...
                    Log.logDebug("NetworkEventHandler.readEvent(): failed to read from " + str(self.transp) + ":" + str(
                        self.ip) + ":" + str(self.port) + ": " + str(inst), 1)
                    raise inst
            else:
                # Log.logTest("WARNING: network timeout after " + str(self.timeout) + " seconds")
                Log.logDebug(
                    "NetworkEventHandler.readEvent(): WARNING: timeout after " + str(self.timeout) + " seconds", 1)
                return None
            ...
            if len(s_in) > 0:
                if self.state == NetworkEventHandler.NEH_LISTEN:
                    ...
                    if hasattr(Config, "resources"):
                        key = str(getTransportNumber(self.transp)) + str(self.ip) + str(self.port) + str(
                            self.remoteip) + str(self.remoteport)
                        Config.resources['NEH'][key] = self
                elif self.state == NetworkEventHandler.NEH_CONNECTED:
    ...
    def writeEvent(self, event):
        ...
        if (self.transp == socket.SOCK_DGRAM):
            Log.logDebug("Sending UDP message from " + str(self.ip) + ":" + str(self.port) + " to " + str(
                dstAddr[0]) + ":" + str(dstAddr[1]) + " ...", 1)
            ...
            Log.logDebug("Sending TCP message from " + str(self.ip) + ":" + str(self.port) + " to " + str(
                self.remoteip) + ":" + str(self.remoteport) + " ...", 1)
    ...
    while not success:
        ...
        try:
            ...
            try:
                ...
                if (port % 2 == 0):
                    Log.logDebug(
                        "Media sockets bound, RTP: " + str(ip) + ":" + str(port) + ", RTCP: " + str(ip) + ":" + str(
                            port2), 1)
                    ret_pair = (sock, sock2)
                    success = True
                else:
