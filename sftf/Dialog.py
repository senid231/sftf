...
from sftf import Config, Log, Helper
import Helper
    ...
    def buildRouteSet(self, isClient):
        ...
        if isClient:
            ...
            if self.transaction[0].lastReply.code >= 300:
                Log.logDebug("Dialog.buildRouteSet(): lastReply code (\'" + str(
                    self.transaction[0].lastReply.code) + "\') is negative => won't build a RouteSet", 2)
            ...
            if self.transaction[0].lastReply.hasParsedHeaderField("Record-Route"):
                ...
                curRR = RR
                if curRR.__next__ is None:
                    # only on RR element nothing to invert
                    self.RouteSet = RR
                else:
                    # copy first element
                    while curRR.next.__next__ is not None:
                        curRR = curRR.__next__
                    self.RouteSet = copy.deepcopy(curRR.__next__)
                    curRR.next = None
                    curRR = RR
                    # copy all middle elements
                    RS = self.RouteSet
                    while curRR.__next__ is not None:
                        while curRR.next.__next__ is not None:
                            curRR = curRR.__next__
                        RS.next = copy.deepcopy(curRR.__next__)
                        RS = RS.__next__
                        curRR.next = None
                        curRR = RR
                    # copy last element
    ...
    def appendTransaction(self, trans):
        ...
        if (len(self.transaction) == 0):
            ...
            if trans.message[0].hasParsedHeaderField("To"):
                ...
                Log.logDebug(
                    "Dialog.appendTransaction(): WRNING: parsed To missing, using raw To instead (can be empty)", 2)
            ...
            if trans.message[0].hasParsedHeaderField("From"):
                ...
                Log.logDebug("Dialog.appendTransaction(): parsed From missing, using raw From instead (can be empty)",
                             2)
            ...
            if trans.isClient:
                ...
                if trans.message[0].hasParsedHeaderField("Contact"):
                    ...
                    Log.logDebug(
                        "Dialog.appendTransaction(): parsed Contact missing, using raw Contact instead (can be empty)",
                        2)
            ...
            if trans.message[0].hasParsedHeaderField("Call-ID"):
                ...
                Log.logDebug(
                    "Dialog.appendTransaction(): parsed Call-ID missing, using raw Call-ID instead (can be empty)", 2)
