...
from sftf import FileEventHandler, Config, Log, Helper
...
class TestCase:
    ...
    def addResource(self, resource):
        ...
        if isinstance(resource, NetworkEventHandler):
            ...
            Log.logDebug(
                "TestCase.addResource(): adding resource to FEH list with key '" + str(resource.filename) + "'", 5)
    ...
    def findNEH(self, _transport, _lip, _lport, _rip, _rport):
        ...
        """
        Log.logDebug(
            "TestCase.findNEH(): called with transport: " + str(_transport) + " lip: " + str(_lip) + " lport: " + str(
                _lport) + " rip: " + str(_rip) + " rport: " + str(_rport), 5)
        # ret = []
        if 'NEH' in Config.resources:
            key = str(getTransportNumber(_transport)) + str(_lip) + str(_lport)
            if _transport.lower() == "udp":
                key += "00"
            elif _transport.lower() == "tcp":
                key += _rip + _rport
            if key in Config.resources['NEH']:
    ...
    def findFEH(self, _filename):
        ...
        """
        # ret = []
        if 'FEH' in Config.resources:
            if _filename in Config.resources['FEH']:
            ...
            if (message.hasParsedHeaderField("Call-ID")):
                ...
                if cur_dia is None:
                    if self.ignoreCallID:
                        Log.logDebug(
                            "TestCase.addMessage(): ignoring Call-ID as requested => appending message at last dialog",
                            2)
                    ...
                    if via.branch.startswith("z9hG4bK-SCb-"):
                        ...
                        if numend != -1:
                            ...
                            if tr_num <= len(cur_dia.transaction):
                                if message.hasParsedHeaderField("CSeq") and (
                                            message.getParsedHeaderValue("CSeq").number == cur_dia.transaction[
                                            tr_num].CSeq.number) and (cur_dia.transaction[tr_num].isClient != recv):
                        ...
                        for i in cur_dia.transaction:
                            if (i.message[0] is not None) and (i.message[0].hasParsedHeaderField("Via")) and (
                                        i.message[0].getParsedHeaderValue("Via").branch is not None) and (
                                        i.message[0].getParsedHeaderValue("Via").branch == via.branch):
                                # to protect us from broken implementations
                                # which reuse same via branch for several
                                # transactions check the CSeq number too
                                if message.hasParsedHeaderField("CSeq") and (
                                            message.getParsedHeaderValue("CSeq").number != i.CSeq.number):
                                    Log.logTest(
                                        "re-use of Via branch value for different transactions is not RFC3261 compliant!!!")
                                    Log.logDebug(
                                        "TestCase.addMessage(): re-use of Via branch value for different transactions is not RFC3261 compliant!!!",
                                        1)
    ...
    def createChallenge(self, mes=None, realm=None, qop=False, algorithm="MD5", stale=None, proxy=False):
        ...
        if proxy:
            ...
            if repl is None:
                raise SCException("TestCase", "createChallenge",
                                  "createReply(407, \"Proxy Authentication Required\", createEvent=False) failed")
            ...
            if repl is None:
                raise SCException("TestCase", "createChallenge",
                                  "createReply(401, \"Unauthorized\", createEvent=False) failed")
    ...
    def checkAuthResponse(self, mes=None, username=None, password=None, realm=None, proxy=False):
        ...
        if cr == sr:
            ...
            Log.logTest(
                "authentication failure: client reponse (\'" + cr + "\') does not match calculated response (\'" + sr + "\')")
            return False

    def readMessageFromNetwork(self, _NetworkEventHandler=None, StartTimeout=None, CatchTimeout=True, TimeoutError=True,
                               ignoreRequests=False, ignoreReplies=False, RequestMethod=None):
        ...
        if _NetworkEventHandler is None:
            ...
            neh = self.findNEH(self.transport, Config.LOCAL_IP, Config.LOCAL_PORT, Config.TEST_HOST,
                               Config.TEST_HOST_PORT)
            if neh is None:
        ...
        if ignoreRequests and ignoreReplies:
            Log.logDebug(
                "TestCase.readMessageFromNetwork(): WARNING (implementation bug?):ignoring requests and replis is not reasonable, ignoring options",
                1)
            ignoreRequests = ignoreReplies = False
        if (not self.autoReact) and (ignoreRequests or ignoreReplies):
            Log.logDebug(
                "TestCase.readMessageFromNetwork(): WARNING: ignoreRequests or ignoreReplies is on, but automatic reaction is turned off, reasonable?",
                1)
        if (not RequestMethod is None) and (ignoreRequests is True):
            Log.logDebug(
                "TestCase.readMessageFromNetwork(): WARNING: ignoreRequests AND RequestMethod set, senceless, ignoring RequestMethod",
                1)
            RequestMethod = None
        ev = None
        evtime = None
        ...
        while ev is None:
            while (_NetworkEventHandler.timeout <= MaxTimeout) and (ev is None):
                try:
                    ...
                    if inst[0] == 10054:
                        self.addResult(TestResult.TR_ERROR,
                                       "received ICMP error from remote for the last message while trying to read")
                        return None
                    else:
                        raise inst
                if ev is None:
            ...
            if ev is None:
                ...
                if self.name in Config.resources['XMLEH']:
                ...
                if self.ignoreRetrans and retrans:
                    ...
                    if mes.isRequest and ignoreRequests:
                        ...
                        ev = None
                    elif mes.isRequest and (not RequestMethod is None) and (
                                mes.method.lower() != RequestMethod.lower()):
                        Log.logDebug("TestCase.readMessageFromNetwork(): ignoring unwatend request type '" + str(
                            mes.method) + "'", 4)
                        ev = None
                    else:
                        return mes
            if not evtime is None:
                ...
                if elapsed > MaxTimeout:
                    Log.logDebug("TestCase.readMessageFromNetwork(): elapsed time is bigger then Maxtimeout => timeout",
                                 5)
                    return None
    ...
    def readReplyFromNetwork(self, _NetworkEventHandler=None, StartTimeout=None, CatchTimeout=True, TimeoutError=True):
        ...
        """
        return self.readMessageFromNetwork(_NetworkEventHandler, StartTimeout, CatchTimeout, TimeoutError,
                                           ignoreRequests=True, ignoreReplies=False, RequestMethod=None)

    def readRequestFromNetwork(self, _NetworkEventHandler=None, StartTimeout=None, CatchTimeout=True, TimeoutError=True,
                               RequestMethod=None):
        ...
        """
        return self.readMessageFromNetwork(_NetworkEventHandler, StartTimeout, CatchTimeout, TimeoutError,
                                           ignoreRequests=False, ignoreReplies=True, RequestMethod=RequestMethod)
    ...
    def setMediaBody(self, mes=None, sock=None, checksend=True):
        ...
        if checksend and (mes is not None) and (not mes.isRequest) and (not mes.request is None) and (
                not mes.request.parsedBody is None) and (len(mes.request.parsedBody.media) == 0):
    ...
    def createRequest(self, method, ruri=None, trans=None, dia=None, createEvent=True, neh=None):
        ...
        and transaction.
        """
        Log.logDebug("TestCase.createRequest(): entered with method=\'" + str(method) + "\', ruri=\'" + str(
            ruri) + "\', trans=\'" + str(id(trans)) + "\', dia=\'" + str(id(dia)) + "\', createEvent=\'" + str(
            createEvent) + "\'", 5)
        if (method.lower() == "bye") and (dia is None):
            Log.logDebug(
                "TestCase.createRequest(): WARNING creating BYE without dialog!!! You should specify the dialog in the test to send the BYE for!",
                1)
        if (method.lower() == "cancel" or method.lower() == "ack") and (trans is None):
            Log.logDebug(
                "TestCase.createRequest(): WARNING creating ACK or CANCEL without transaction!!! You should specify the transaction in the test to send the ACK or CANCEL for!",
                1)
        route = None
        dsthost = None
        ...
        if dia is None:
            ...
            if dia is None:
                ...
                if ruri is None:
                    ...
                    raise SCException("TestCase", "createRequest",
                                      "creating a request from a dialog with a given request-uri is not supported")
        else:
            if ruri is None:
                ruri, route, dsthost = dia.getRoutingTarget()
            else:
        ...
        if (trans is None) and ((method.lower() == "ack") or (method.lower() == "cancel")):
            ...
            if (trans is None) and ((method.lower() == "ack") or (method.lower() == "cancel")):
                raise SCException("TestCase", "createRequest",
                                  "creating ACK or CANCEL without transaction is not possible")
        req = SipRequest()
        req.method = method
        req.protocol = "SIP"
        req.version = "2.0"
        ...
        elif (method.lower() == "cancel") and (trans.firstRequest is not None) and (
                trans.firstRequest.hasParsedHeaderField("To")):
        ...
        if dia.CallID is None:
            req.setHeaderValue("Call-ID",
                               str(id(dia)) + "-" + str(hex(int(time.time())))[2:] + "@" + Config.LOCAL_IP + "\r\n")
        else:
        ...
        if (method.lower() == "cancel") or (method.lower() == "ack"):
            ...
            req.setHeaderValue("Via", "SIP/2.0/" + neh.getTransport() + " " + str(neh.localip) + ":" + str(
                neh.localport) + "\r\n")
        else:
            req.setHeaderValue("Via", "SIP/2.0/" + self.transport + " " + str(Config.LOCAL_IP) + ":" + str(
                Config.LOCAL_PORT) + "\r\n")
    ...
    def writeMessageToNetwork(self, _NetworkEventHandler=None, Message=None):
        ...
        if (Message.event is not None) and Message.event.received:
            raise SCException("TestCase", "writeMessageToNetwork",
                              "last message is received, wont send received messages")
        if self.fixHeaders:
            ...
            if dia is None:
                ...
                dia.localUri.tag = "SCt-" + str(dia.number) + "-" + str(
                    self.getUniqueTimestamp()) + "-" + Config.LOCAL_IP + "~" + str(self.__class__.__name__)
            if trans is None:
            ...
            if isinstance(Message, SipRequest):
                ...
                raise SCException("TestCase", "writeMessageToNetwork",
                                  "unsupported type of message to lookup the mandatory HFH")
            if man_hfh is None:
                ...
                for hfh in man_hfh:
                    if not Message.hasParsedHeaderField(hfh):
                        Log.logDebug(
                            "TestCase.writeMessageToNetwork(): WARNING: message misses mandatory header " + hfh, 1)
                    ...
                    if hfh == "Via":
                        ...
                        if mes_req:
                            if via.branch is None:
                                via.branch = "z9hG4bK-SCb-" + str(trans.number) + "-" + str(
                                    self.getUniqueTimestamp()) + "-" + Config.LOCAL_IP + "~" + str(
            ...
            if mes_req:
                if dia.RouteSet is not None:
                    if not Message.hasParsedHeaderField("Route"):
                        ...
                        Message.setEventAddresses(
                            (Config.LOCAL_IP, int(Config.LOCAL_PORT), _NetworkEventHandler.transp), dsthost)
            else:
                # FIXME add RR header for < 300
                Message.setEventAddresses((Message.request.event.dstAddress[0], Message.request.event.dstAddress[1],
                                           _NetworkEventHandler.transp), Message.request.getReplyAddress())
            Message.createEvent()
    ...
    def reactOn(self, message):
        ...
        if hasattr(self, on_str):
            ...
            if message.isRequest:
                Log.logDebug("TestCase.reactOn(): no method found for \'" + on_str + "\', calling onDefaultMethod...",
                             1)
    ...
    def onDefaultCode(self, message):
        ...
        """
        if message.code >= 200:
            if (message.hasParsedHeaderField("CSeq") and (message.getParsedHeaderValue("CSeq").method == "INVITE")) or (
                        message.transaction.firstRequest.method == "INVITE"):
