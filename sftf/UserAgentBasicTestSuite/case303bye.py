...
# MA  02111-1307  USA
#
# $Id: case303bye.py,v 1.2 2004/05/02 18:57:35 lando Exp $
#
import sftf.NetworkEventHandler as NEH
from sftf import Log, TestCase


class case303bye(TestCase):
    def config(self):
        self.name = "Case 303bye"
        self.description = "Digest Authentication of BYE with qop"
        self.isClient = True
        self.transport = "UDP"
    ...
    def run(self):
        ...
        # if not self.userInteraction("case303bye: proceed when ready to send a INVITE"):
        # neh.closeSock()
        #	return
        ...
        if req is None:
            ...
            if self.challenged == 1:
                if req.hasParsedHeaderField("Authorization"):
                    ...
                    if ret:
                        Log.logDebug("case303bye: warnings or errors about the Authorization header, see test log", 1)
                        Log.logTest("case303bye: warnings or errors about the Authorization header, see WARNINGS above")
                        self.results.extend(ret)
                    if self.first_bye.hasParsedHeaderField("CSeq") and self.bye.hasParsedHeaderField("CSeq"):
                        if (self.bye.getParsedHeaderValue("CSeq").number <= self.first_bye.getParsedHeaderValue(
                                "CSeq").number) and (
                                    self.bye.getParsedHeaderValue("CallID") == self.first_bye.getParsedHeaderValue(
                                        "CallID")):
                            self.addResult(TestCase.TC_WARN, "CSeq number was not increased for authorization")
                else:
                    if req.hasHeaderField("Authorization"):
                        Log.logDebug("case303bye: failed to parse the given Authorization header", 1)
                        Log.logTest("case303bye: unable to parse the Authorization header")
                        self.addResult(TestCase.TC_ERROR, "failed to parse Authorization header")
                    else:
                        Log.logDebug("case303bye: missing Authorization header in request", 1)
                        Log.logTest("case303bye: missing Authorization header in request")
                        self.addResult(TestCase.TC_FAILED, "missing Authorization header in request")

                if self.checkAuthResponse(req):
                    Log.logDebug("case303bye: authenticaton reply is valid", 2)
                    Log.logTest("case303bye: authenticaton reply is valid")
                    self.addResult(TestCase.TC_PASSED, "authentication reply is valid")
                else:
                    Log.logDebug("case303bye: authentication reply is NOT valid", 1)
                    Log.logTest("case303bye: authentication reply is NOT valid")
