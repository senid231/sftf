...
# MA  02111-1307  USA
#
# $Id: case215.py,v 1.2 2004/05/02 18:57:35 lando Exp $
#
import sftf.NetworkEventHandler as NEH
from sftf import Log, TestCase


class case215(TestCase):
    def config(self):
        self.name = "Case 215"
        self.description = "Max-Forwards = 0"
        self.isClient = True
        self.transport = "UDP"
    ...
    def onDefaultCode(self, message):
        ...
        if message.code >= 200:
            if message.getParsedHeaderValue("CSeq").method == "INVITE":
                Log.logDebug("case215: sending ACK for >= 200 reply", 3)
                ack = self.createRequest("ACK", trans=message.transaction)
                self.writeMessageToNetwork(self.neh, ack)
            if message.code == 483:
                self.addResult(TestCase.TC_FAILED, "INVITE rejected with 483")
            elif message.code == 200:
                if message.transaction.canceled:
                    Log.logDebug("case215: received 200 for CANCEL", 3)
                else:
                    Log.logDebug("case215: sending BYE for accepted INVITE", 3)
                ...
                self.addResult(TestCase.TC_FAILED, "INVITE rejected, but not with 483")
        else:
            self.addResult(TestCase.TC_PASSED, "INVITE accepted")
