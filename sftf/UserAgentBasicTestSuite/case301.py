...
from sftf import Config, Log, TestCase
        ...
        if reg is None:
            ...
            if reg.hasParsedHeaderField("Authorization"):
                ...
                if reg.hasParsedHeaderField("CSeq") and req.hasParsedHeaderField("CSeq"):
                    if (reg.getParsedHeaderValue("CSeq").number <= req.getParsedHeaderValue("CSeq").number) and (
                                reg.getParsedHeaderValue("CallID") == req.getParsedHeaderValue("CallID")):
            ...
            if reg.hasParsedHeaderField("From"):
                ...
                if (not NEH.compareHostNames(fm.uri.host, Config.LOCAL_IP)) and (
                        not NEH.compareHostNames(fm.uri.host, Config.LOCAL_HOSTNAME)):
