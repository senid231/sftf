...
# MA  02111-1307  USA
#
# $Id: case201.py,v 1.2 2004/05/02 18:57:35 lando Exp $
#
import sftf.NetworkEventHandler as NEH
from sftf import Log, TestCase


class case201(TestCase):
    def config(self):
        self.name = "Case 201"
        self.description = "A short torture request"
        self.isClient = True
        self.transport = "UDP"
    ...
    def onDefaultCode(self, message):
        ...
        if message.code >= 200:
            if message.getParsedHeaderValue("CSeq").method == "INVITE":
                Log.logDebug("case201: sending ACK for >= 200 reply", 3)
                ...
                if message.transaction.canceled:
                    Log.logDebug("case201: received 200 for CANCEL", 3)
                else:
                    Log.logDebug("case201: sending BYE for accepted INVITE", 3)
            ...
            self.addResult(TestCase.TC_PASSED, "torture INVITE accepted")
