...
class case228(TestCase):
    ...
    def run(self):
        ...
        to.displayname = "SC@SIP-Forum".encode('utf8')
        ...
        fr.displayname = str("\xC4\xD6\xDC\xE4\xF6\xFC\xDF", "unicode_escape").encode('utf8')
    ...
    def on180(self, message):
        ...
        self.addResult(TestCase.TC_INIT,
                       "the From contained german umlauts: A\"O\"U\"a\"o\"u\"sz (verify the UA missed call log)")
        ...
        if (message.code >= 300):
            if (message.getParsedHeaderValue("CSeq").method == "INVITE"):
                self.addResult(TestCase.TC_WARN,
                               "INVITE with UTF displaynames rejected with '" + str(message.code) + "'")
                ack = self.createRequest("ACK", trans=message.transaction)
                self.writeMessageToNetwork(self.neh, ack)
                self.end = 1
