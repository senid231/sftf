...
from sftf import Log, Helper, TestCase
from sftf.SCException import SCException
...
class case401(TestCase):
    ...
    def run(self):
        ...
        try:
            ...
            Log.logDebug("case401: timeout after " + str(
                1.2 * self.reregister_time) + "sec, allthough Contact expired after " + str(
                self.reregister_time) + "sec", 1)
        ...
        if (delay > (self.reregister_time * 0.25)) and (delay < (self.reregister_time * 0.75)):
            Log.logDebug(
                "case401: delay between REGISTER is between 0.25 * expires and 0.75 * expires => additional load", 2)
            ...
            Log.logDebug(
                "case401: delay between re-REGISTER was " + str(delay) + " sec, which is above the expires value", 1)
            ...
            Log.logDebug("case401: delay between re-REGISTER was " + str(
                delay) + " sec, which is below 1/4 of the expires value", 1)
