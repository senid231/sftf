...
class case510(TestCase):
    ...
    def on200(self, message):
        ...
        if self.reinvited == 0:
            ...
            if (self.rereinvited == 0) and (self.reinvited == 1) and (
    ...
    def onDefaultCode(self, message):
        if (message.hasParsedHeaderField("CSeq")) and (message.getParsedHeaderValue("CSeq").method == "INVITE") and (
                    message.code >= 300):
        ...
        if self.reinvited == 1:
            self.addResult(TestCase.TC_ERROR,
                           "re-INVITE with Require:FooBar rejected with \'" + str(message.code) + "\'")
        if self.rereinvited == 1:
            self.addResult(TestCase.TC_FAILED,
                           "re-INVITE after re-INVITE with Require:FooBar rejected with \'" + str(message.code) + "\'")
        if self.byed == 0:
            bye = self.createRequest("BYE", dia=self.fst_inv.transaction.dialog)
            self.byed = 1
            self.writeMessageToNetwork(self.neh, bye)
