...
from sftf.Helper import eventTimeDiff
...
class case605c(TestCase):
    ...
    def run(self):
        ...
        if repl_a_l > 1:
            time_array = []
            num_len = list(range(0, repl_a_l - 1))
            for i in num_len:
            ...
            if time_array[0] < (min_percent * 0.5):
                self.addResult(TestCase.TC_FAILED,
                               "timer for first retry (T1=0.5s) is below " + str(min_percent * 0.5) + " (" + str(
                                   time_array[0]) + ")")
                passed = False
            elif time_array[0] > (max_percent * 0.5):
                self.addResult(TestCase.TC_FAILED,
                               "timer for first retry (T1=0.5s) is above " + str(max_percent * 0.5) + " (" + str(
                                   time_array[0]) + ")")
                passed = False
            if len(time_array) > 4:
                f_ran = list(range(0, 4))
                t_ran = list(range(4, len(time_array) - 1))
                for i in f_ran:
                    if time_array[i + 1] < 2 * time_array[i] * min_percent:
                        self.addResult(TestCase.TC_FAILED,
                                       "timer for retry " + str(i + 1) + " is less than doubled (" + str(
                                           time_array[i]) + ", " + str(time_array[i + 1]) + ")")
                        passed = False
                    elif time_array[i + 1] > 2 * time_array[i] * max_percent:
                        self.addResult(TestCase.TC_FAILED,
                                       "timer for retry " + str(i + 1) + " is more than doubled (" + str(
                                           time_array[i]) + ", " + str(time_array[i + 1]) + ")")
                        passed = False
                for j in t_ran:
                    if time_array[j] < 4.0 * min_percent:
                        self.addResult(TestCase.TC_FAILED,
                                       "timer for retry " + str(j + 1) + " is less than T2 (=" + str(
                                           4.0 * min_percent) + "s) (" + str(time_array[j]) + ")")
                        passed = False
                    elif time_array[j] > 4.0 * max_percent:
                        self.addResult(TestCase.TC_FAILED,
                                       "timer for retry " + str(j + 1) + " is more than T2 (=" + str(
                                           4.0 * max_percent) + "s) (" + str(time_array[j]) + ")")
                        passed = False
            else:
                num_len2 = list(range(0, len(time_array) - 1))
                for i in num_len2:
                    if time_array[i + 1] < 2 * time_array[i] * min_percent:
                        self.addResult(TestCase.TC_FAILED,
                                       "timer for retry " + str(i + 1) + " is less than doubled (" + str(
            ...
            if t_sum < (min_percent * 63 * time_array[0]):
                self.addResult(TestCase.TC_FAILED, "last retry (" + str(t_sum) + ") is below 63*T1 (=" + str(
                    min_percent * 63 * time_array[0]) + ")")
                passed = False
            elif t_sum > (max_percent * 63 * time_array[0]):
                self.addResult(TestCase.TC_FAILED, "last retry (" + str(t_sum) + ") is above 63*T1 (=" + str(
                    min_percent * 63 * time_array[0]) + ")")
                passed = False
            if passed:
