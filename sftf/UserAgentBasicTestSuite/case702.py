    ...
    def run(self):
        ...
        if self.domain == "":
            ...
            if ld == -1:
                ...
                if ld == -1:
                    Log.logTest(
                        "case702: failed to get the domain name: please fix the DNS setup or try to add a domain (by editing the case702.py file)")
                    return
            ...
            if dns_rsp is None:
                ...
                if rd2 == -1:
                    ...
                    Log.logTest(
                        "case702: missing DNS SRV records for '" + self.srv_que + "' and automatic domain detection failed")
                    Log.logTest(
                        "case702: please adjust the domain and transport values in the case702.py file if you have a working DNS SRV setup")
            ...
            if dns_rsp is None:
                ...
                Log.logTest(
                    "case702: please add SRV records to your DNS setup or adjust the domain value to the correct value")
                self.addResult(TestCase.TC_ERROR, "missing DNS SRV records for '" + self.srv_que + "'")
                return

        if len(dns_rsp) != 2:
            Log.logTest("case702: for this test we need exactly two SRV records, currently " + str(
        ...
        for srv in dns_rsp:
            ...
            if str(srv.target) != hostname:
                ...
                Log.logTest(
                    "case702: one entry (" + str(srv) + ") is not pointing at this host (" + str(hostname) + ")")
