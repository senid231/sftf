...
class case905(TestCase):
    ...
    def run(self):
        ...
        if (self.dialog[0].transaction[0].canceled and (self.dialog[0].transaction[0].firstACK is None)) or (
                    (self.dialog[0].transaction[0].firstReply is not None) and (
                            self.dialog[0].transaction[0].firstReply.code != 487)):
                    ...
                    if new_rep != self.replied:
                        ...
                        if message.code != 482:
                            self.addResult(TestCase.TC_WARN,
                                           "the second forked INVITE should be rejected with 482, instead received '" + str(
                                               message.code) + "'")
                    else:
                        Log.logTest("received negative reply \'" + str(
                            message.code) + "\' for a different branch then the first 180")
