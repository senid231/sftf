...
# MA  02111-1307  USA
#
# $Id: case216.py,v 1.2 2004/05/02 18:57:35 lando Exp $
#
import sftf.NetworkEventHandler as NEH
from sftf import Log, TestCase


class case216(TestCase):
    def config(self):
        self.name = "Case 216"
        self.description = "Unacceptable Accept offering"
        self.isClient = True
        self.transport = "UDP"
    ...
    def run(self):
        ...
        ac = self.getParsedHeaderInstance("Accept")
        ac.type = "text"
        ac.subtype = "unacceptablesubtype"
        inv.setParsedHeaderValue("Accept", ac)
        inv.setHeaderValue("Accept", ac.create())
    ...
    def onDefaultCode(self, message):
        ...
        if message.code >= 200:
            if message.getParsedHeaderValue("CSeq").method == "INVITE":
                Log.logDebug("case216: sending ACK for >= 200 reply", 3)
                ack = self.createRequest("ACK", trans=message.transaction)
                self.writeMessageToNetwork(self.neh, ack)
            if (message.code == 400) or (message.code == 406):
                self.addResult(TestCase.TC_PASSED, "INVITE rejected with 400 or 406")
            elif message.code == 200:
                if message.transaction.canceled:
                    Log.logDebug("case216: received 200 for CANCEL", 3)
                else:
                    Log.logDebug("case216: sending BYE for accepted INVITE", 3)
                ...
                self.addResult(TestCase.TC_FAILED, "INVITE rejected, but not with 400 or 406")
        else:
            self.addResult(TestCase.TC_FAILED, "INVITE accepted, not rejected with 400 or 406")
