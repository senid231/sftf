...
# MA  02111-1307  USA
#
# $Id: case208exp.py,v 1.2 2004/05/02 18:57:35 lando Exp $
#
import sftf.NetworkEventHandler as NEH
from sftf import Log, TestCase

# FIXME:
# - should we insist on a 400 or also accpet 180
# - should we add Max-Forwards 300 here too?

class case208exp(TestCase):
    def config(self):
        self.name = "Case 208exp"
        self.description = "Request scalar fields with overlarge values (Expires)"
        self.isClient = True
        self.transport = "UDP"
    ...
    def run(self):
        ...
        ex = self.getParsedHeaderInstance("Expires")
        ex.seconds = 4294967297
        inv.setParsedHeaderValue("Expires", ex)
        inv.setHeaderValue("Expires", ex.create())
    ...
    def onDefaultCode(self, message):
        ...
        if message.code >= 200:
            if message.getParsedHeaderValue("CSeq").method == "INVITE":
                Log.logDebug("case208exp: sending ACK for >= 200 reply", 3)
            ...
            if message.code == 400:
                ...
                if message.transaction.canceled:
                    Log.logDebug("case208exp: received 200 for CANCEL", 3)
                else:
                    Log.logDebug("case208exp: sending BYE for accepted INVITE", 3)
