    ...
    def run(self):
        ...
        if req_a_l != 7:
            self.addResult(TestCase.TC_FAILED,
                           "instead of 7 INVITE requests " + str(req_a_l) + " requests have been received")
            passed = False
        if req_a_l > 1:
            time_array = []
            num_len = list(range(0, req_a_l - 1))
            for i in num_len:
            ...
            for j in num_len2:
                if time_array[j + 1] < 2 * time_array[j] * min_percent:
                    self.addResult(TestCase.TC_FAILED,
                                   "timer for retry " + str(j + 1) + " is less then doubled (" + str(
                                       time_array[j]) + ", " + str(time_array[j + 1]) + ")")
                    passed = False
                elif time_array[j + 1] > 2 * time_array[j] * max_percent:
                    self.addResult(TestCase.TC_FAILED,
                                   "timer for retry " + str(j + 1) + " is more then doubled (" + str(
                                       time_array[j]) + ", " + str(time_array[j + 1]) + ")")
                    passed = False
            t_sum = 0.0
            for k in time_array:
