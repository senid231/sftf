    ...
    def run(self):
        ...
        if self.domain == "":
            ...
            if ld == -1:
                ...
                if ld == -1:
                    Log.logTest(
                        "case703: failed to get the domain name: please fix the DNS setup or try to add a domain (by editing the case703.py file)")
                    return
            ...
            if dns_rsp is None:
                ...
                if rd2 == -1:
                    ...
                    Log.logTest(
                        "case703: missing DNS SRV records for '" + self.srv_que + "' and automatic domain detection failed")
                    Log.logTest(
                        "case703: please adjust the domain and transport values in the case703.py file if you have a working DNS SRV setup")
            ...
            if dns_rsp is None:
                ...
                Log.logTest(
                    "case703: please add SRV records to your DNS setup or adjust the domain value to the correct value")
                self.addResult(TestCase.TC_ERROR, "missing DNS SRV records for '" + self.srv_que + "'")
                return

        if len(dns_rsp) != 2:
            Log.logTest("case703: for this test we need exactly two SRV records, currently " + str(
                len(dns_rsp)) + " records are configured")
            self.addResult(TestCase.TC_ERROR,
                           "wrong number (" + str(len(dns_rsp)) + ")of SRV records for this test (exactly 2 required)")
            return
        for srv in dns_rsp:
            ...
            if str(srv.target) != hostname:
                ...
                Log.logTest(
                    "case703: one entry (" + str(srv) + ") is not pointing at this host (" + str(hostname) + ")")
        ...
        if req is None:
            ...
            if req2 is None:
                ...
                if req3 is None:
                    self.addResult(TestCase.TC_FAILED, "did not received REGISTER at primary socket within " + str(
                        3 * self.expires) + " seconds")
                else:
                    ...
                    if (test is not None) and (test.method == "REGISTER"):
                        self.addResult(TestCase.TC_WARN,
                                       "disregarded SRV priorities and tried to re-register at backup server first")
                    diff = int(Helper.eventTimeDiff(req3.event, req2.event))
                    if diff > self.expires:
                        self.addResult(TestCase.TC_WARN, "re-registration at primary server was " + str(
                            diff - self.expires) + " seconds after expiration of Contact at backup server")
