...
# MA  02111-1307  USA
#
# $Id: case214.py,v 1.2 2004/05/02 18:57:35 lando Exp $
#
    ...
    def onDefaultCode(self, message):
        ...
        if message.code >= 200:
            if message.getParsedHeaderValue("CSeq").method == "INVITE":
                Log.logDebug("case214: sending ACK for >= 200 reply", 3)
                ack = self.createRequest("ACK", trans=message.transaction)
                self.writeMessageToNetwork(self.neh, ack)
            if message.code == 415:
                self.addResult(TestCase.TC_PASSED, "INVITE rejected with 415")
            elif message.code == 200:
                if message.transaction.canceled:
                    Log.logDebug("case214: received 200 for CANCEL", 3)
                else:
                    Log.logDebug("case214: sending BYE for accepted INVITE", 3)
                ...
                self.addResult(TestCase.TC_FAILED, "INVITE rejected, but not with 415")
        else:
            self.addResult(TestCase.TC_FAILED, "INVITE accepted, not rejected with 415")
