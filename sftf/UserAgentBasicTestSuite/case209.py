...
# MA  02111-1307  USA
#
# $Id: case209.py,v 1.2 2004/05/02 18:57:35 lando Exp $
#
import sftf.NetworkEventHandler as NEH
from sftf import Log, TestCase


class case209(TestCase):
    def config(self):
        self.name = "Case 209"
        self.description = "Unterminated quoted string in display name"
        self.isClient = True
        self.transport = "UDP"
    ...
    def run(self):
        ...
        to.displayname = "Foo Bar"
        inv.setHeaderValue("To", to.create().replace("\"Foo Bar\"", "\"Foo Bar"))
    ...
    def onDefaultCode(self, message):
        ...
        if message.code >= 200:
            if message.getParsedHeaderValue("CSeq").method == "INVITE":
                Log.logDebug("case209: sending ACK for >= 200 reply", 3)
            ...
            if message.code == 400:
                ...
                if message.transaction.canceled:
                    Log.logDebug("case209: received 200 for CANCEL", 3)
                else:
                    Log.logDebug("case209: sending BYE for accepted INVITE", 3)
