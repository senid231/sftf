...
#
from sftf import SipEvent, Log, Helper
from sftf.TestResult import TestResult
from sftf.SCException import SCException, SCNotImplemented
...
class SipMessage:
    ...
    def getHeaderValue(self, hfName):
        ...
        """
        if hfName.replace("-", "").capitalize() in self.headerFields:
    ...
    def parseHeaderField(self, headerStr, checkHeaderNames):
        ...
        try:
            ...
            Log.logDebug("SipMessage.parseHeaderField(): failed to find the name-value sperator in: " + str(headerStr),
                         2)
            return ret
        if checkHeaderNames:
            ...
            if hfn_raw.capitalize() in Helper.rfc_headers:
                if not (hfn_raw in Helper.rfc_bfn_headers):
                    ret.append(TestResult(TestResult.TR_CAUT, "header name '" + str(
                        hfn_raw) + "' is not exactly written as in RFC3261's BNF"))
            else:
                Log.logDebug(
                    "SipMessage.parseHeaderFiled(): header field name '" + str(hfn_raw) + "' is not in RFC3261", 2)
        hfn = Helper.getMappedHFH(headerStr[0:index].rstrip())
        if (hfn in self.headerFields and (hfn in Helper.multiple_headers)):
    ...
    def createEvent(self):
        ...
        hf_len = list(range(0, len(self.headerFields)))
        hf = list(self.headerFields.items())
        for i in hf_len:
            att, val = hf[i]
    ...
    def hasParsedHeaderField(self, hfName):
        ...
        if hfn_m is None:
            ...
            hfn_m = hfName
        return hfn_m in self.parsedHeader
    ...
    def removeParsedHeaderField(self, hfName):
        ...
        """
        if hfName.replace("-", "").capitalize() in self.parsedHeader:
    ...
    def setEventAddresses(self, Src, Dst):
        ...
        if len(Src) != 3:
            raise SCException("SipMessage", "setEventAddresses",
                              "source address requires 3 parameter, got: " + str(Src))
        if len(Dst) != 2:
            raise SCException("SipMessage", "setEventAddresses",
                              "destination address requires 2 parameter, got: " + str(Dst))
